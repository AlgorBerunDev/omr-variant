...
start_ends = []
for i in range(1,15):
  img = load_and_resize_image(f"omr-images/{i}.jpg", 720)
  ...
  # Создайте ядро для дилатации  Dilatatsiya uchun yadro yarating
...
def is_square(cnt):
  ...
  # Agar taxminiy yo'lda 4 ta burchak bo'lsa, u kvadrat yoki to'rtburchak bo'lishi mumkin
  if len(approx) == 4:
      # Вычисляем площадь контура  Kontur maydonini hisoblang
      area = cv2.contourArea(cnt)

      # Вычисляем площадь ограничивающего прямоугольника  Cheklovchi to'rtburchakning maydonini hisoblang
      x, y, w, h = cv2.boundingRect(approx)
      rect_area = w * h

      # Соотношение площадей  Hudud nisbati
      area_ratio = area / rect_area

      # Соотношение сторон  Aspekt nisbati
      aspect_ratio = float(w) / h

      # Проверяем близость соотношений к 1  Nisbatlarning 1 ga yaqinligini tekshirish
      if 0.95 < area_ratio < 1.05 and 0.95 < aspect_ratio < 1.05:
          ...
          # Barcha tomonlar taxminan teng yoki yo'qligini tekshirish
          if max(edges) / min(edges) < 1.7:
              return True
              # Здесь может быть ваш код для обработки "квадратного" контура
              # Bu erda "kvadrat" konturni boshqarish uchun kodingiz bo'lishi mumkin
  return True
...
for i in range(1,15):
  ...
  list_cnt_area = []
  for contour in contours:
    # Приближаем контуры с помощью многоугольников  Ko'pburchaklar yordamida konturlarga yaqinlashish
    epsilon = 0.05 * cv2.arcLength(contour, True)
    approx = cv2.approxPolyDP(contour, epsilon, True)

    # Если контур имеет 4 вершины, возможно, это прямоугольник
    # Agar konturda 4 ta cho'qqi bo'lsa, u to'rtburchak bo'lishi mumkin
    if len(approx) == 4:
        # Проверим является ли фигура выпуклой  rasmning konveks ekanligini tekshiramiz
  ...
  print('sorted---->>>', sorted(list_cnt_area, reverse=True))
  cv2_imshow(img)
print("Общее время выполнения: {:.2f} секунд".format(total_time))
...
def get_left_max_vertical_cnt(img):
  ...
  # Konturlarni pasayish balandligi bo'yicha tartiblang (to'rtburchaklar oling va balandlikni tortib oling)
  ...
  # Saralashdan keyin dastlabki beshta konturni oling
  top_5_height_contours = sorted_cnts_by_height[:5]

  # Теперь найти контур с максимальной площадью среди этих пяти
  # Endi ushbu beshtadan maksimal maydonga ega bo'lgan konturni toping
...
def remove_without_section_marks(img):
    ...
    max_y = bottom[1] + 10
    min_x = right[0]
    # Получите размеры изображения Rasm o'lchamlarini oling
    height, width = img.shape[:2]

    # Создайте маску, полностью заполненную нулями (черный цвет)
    # To'liq nol bilan to'ldirilgan niqob yarating (qora rang)
    mask = np.ones_like(img) * 0

    # Заполните область между min_y и max_y белым цветом
    # min_y va max_y orasidagi maydonni oq rang bilan to'ldiring
    mask[min_y:max_y, min_x:width] = (255,255,255)

    # Примените маску к изображению, чтобы оставить только область внутри min_y и max_y
    # Faqat min_y va max_y ichidagi maydonni qoldirish uchun rasmga niqob qo'llang
...
for i in range(1,15):
    ...
    height, width = img.shape[:2]
    result_img = img[min_y:max_y, min_x:width]

    gray = cv2.cvtColor(result_img, cv2.COLOR_BGR2GRAY)
    canny_frame = cv2.Canny(gray, 150, 150)
    _, thresh = cv2.threshold(canny_frame, 100, 255, cv2.THRESH_BINARY_INV)
    kernel = np.ones((33,3), np.uint8)
    ...
    # Ищем контуры уже внутри маскированной области
    # Biz niqoblangan maydon ichida allaqachon konturlarni qidirmoqdamiz
    contours, _ = cv2.findContours(thresh, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

    # Теперь у вас есть контуры только внутри заданного прямоугольника
    # Endi sizda faqat berilgan to'rtburchak ichida konturlar mavjud

    # Можете нарисовать их на исходном изображении или на другом холсте
    # Siz ularni asl rasmga yoki boshqa tuvalga chizishingiz mumkin
    canvas = np.zeros((thresh.shape[0], thresh.shape[1], 3), dtype=np.uint8)

    sorted_contours = sorted(contours, key=cv2.contourArea, reverse=True)

    # Оставляем только топ-20 самых больших контуров  Biz faqat eng katta 20 ta eng katta konturni qoldiramiz
    top_contours = sorted_contours[:20]

    # Создаем холст для рисования контуров  Konturlarni chizish uchun kanvas yarating
    canvas = np.zeros((thresh.shape[0], thresh.shape[1], 3), dtype=np.uint8)


    # Рисуем топ-20 контуров  Yuqori 20 ta konturni chizish
    cv2.drawContours(canvas, top_contours, -1, (0, 255, 0), 2)
    # cv2_imshow(canvas)

    for counter in top_contours:
      left, right, top, bottom = get_counter_extreme_points(counter)
      point_x1 = min_x + left[0]
      point_x2 = min_x + right[0]
      cv2.rectangle(img, (point_x1, 0), (point_x2 - 4, height), (0, 255, 0), -1)
    #cv2_imshow(img)
...
for i in range(1,15):
    ...
    # Ищем контуры уже внутри маскированной области  Biz niqoblangan maydon ichida allaqachon konturlarni qidirmoqdamiz
    contours, _ = cv2.findContours(thresh, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    ...
    # Оставляем только топ-5 самых больших контуров
    # Biz faqat eng katta 5 ta konturni qoldiramiz
    top_contours = sorted_contours[:6]

    # Функция для нахождения самой верхней точки контура по оси Y
    # Y o'qi bo'ylab konturning eng yuqori nuqtasini topish funktsiyasi
    def get_topmost_point(contour):
        topmost = tuple(contour[contour[:, :, 1].argmin()][0])
        return topmost[1]

    # Сортируем контуры по самой верхней точке (по оси Y)
    # Konturlarni eng yuqori nuqta bo'yicha tartiblang (Y o'qi bo'ylab)
    top_contours = sorted(top_contours, key=get_topmost_point)[:5]
